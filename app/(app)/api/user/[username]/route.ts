...
export async function GET(
  ...
  try {
    ...
    const { data: profile, error: profileError } = await supabase
      .from('user_profiles')
      .select(
        'user_id, display_name, role, created_at, suspended_until, suspension_reason'
      )
    ...
    const { data: linkedPlayers } = await supabase.rpc(
      'fn_get_linked_players',
      {
    ...
    const totalIncidents =
      linkedPlayers?.reduce(
        (sum: number, player: LinkedPlayer) =>
          sum + (player.incident_count || 0),
        0
      ) || 0
